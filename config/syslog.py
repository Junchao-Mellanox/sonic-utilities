...
@click.option("-c", "--component", required=True, help="Component name in DB for which loglevel is applied (provided with -l)")
@click.option("-l", "--level", required=True, help="Loglevel value", type=click.Choice(['DEBUG', 'INFO', 'NOTICE', 'WARN', 'ERROR']))
@click.option("--service", help="Container name to which the SIGHUP is sent (provided with --pid or --program)")
@click.option("--program", help="Program name to which the SIGHUP is sent (provided with --service)")
@click.option("--pid", help="Process ID to which the SIGHUP is sent (provided with --service if PID is from container)")
@clicommon.pass_db
