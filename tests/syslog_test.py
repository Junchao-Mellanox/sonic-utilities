...
class TestSyslog:
    ...
    def test_config_log_level(self, mock_run):
        ...
        mock_run.return_value = ('something', 0)
        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'component', '-l', 'DEBUG'], obj=db
        )
        assert result.exit_code == SUCCESS

        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'component', '-l', 'DEBUG', '--pid', '123'], obj=db
        )
        assert result.exit_code == SUCCESS

        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'component', '-l', 'DEBUG', '--service', 'pmon', '--pid', '123'], obj=db
        )
        assert result.exit_code == SUCCESS
        
        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'component', '-l', 'DEBUG', '--service', 'pmon', '--program', 'xcvrd'], obj=db
        )
        assert result.exit_code == SUCCESS
    ...
    def test_config_log_level_negative(self, mock_run):
        ...
        mock_run.return_value = ('something', 0)
        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'log1', '-l', 'DEBUG', '--service', 'pmon'], obj=db
        )
        assert result.exit_code != SUCCESS

        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'log1', '-l', 'DEBUG', '--program', 'xcvrd'], obj=db
        )
        assert result.exit_code != SUCCESS

        mock_run.reset_mock()
        result = runner.invoke(
            config.config.commands["syslog"].commands["level"], ['-c', 'log1', '-l', 'DEBUG', '--service', 'swss', '--program', 'orchagent'], obj=db
        )
        assert result.exit_code == SUCCESS
